...
def geometry_aware_assign(predictions, targets, img_w, img_h, valid_mask=None, cfg=None, sample_ys=None):
    ...
    # 扩大Sigma感受野，避免早期训练时大量预测落入高斯平缓尾部导致匹配梯度消失
    score_x = torch.exp(-(delta_x**2) / (2 * 0.15**2))
    score_y = torch.exp(-(delta_y**2) / (2 * 0.2**2))
    score_theta = torch.exp(-(theta_c**2) / (2 * 0.2**2))
    score_length = torch.exp(-(length_c**2) / (2 * 0.3**2))
    
    geom_score = iou_score * score_x * score_y * score_theta * score_length
