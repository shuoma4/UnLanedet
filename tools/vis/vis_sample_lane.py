...
def sample_lane_fixed(points, num_samples):
    ...
    # 4. Spline Interpolation (Use PCHIP to avoid oscillation)
    try:
        # Use PCHIP (Piecewise Cubic Hermite Interpolating Polynomial)
        # It guarantees monotonicity and avoids Runge's phenomenon (oscillation)
        # t (cum_dist) is strictly increasing, so it's valid for PCHIP

        spline_x = PchipInterpolator(cum_dist, points[:, 0])
        spline_y = PchipInterpolator(cum_dist, points[:, 1])
        ...
        print(f'PCHIP interpolation failed: {e}. Fallback to linear.')
...
def visualize_sample_fixed(data_info, idx, num_sample_points, output_dir, img_w, img_h):
    ...
    ax1.imshow(img_rgb)
    ax1.set_title('Original Points', fontsize=12)
    ax2.imshow(img_rgb)
...
def analyze_openlane_slope_std_topk(pkl_path, top_k=50):
    ...
    for sample_id, sample in tqdm(enumerate(data_infos), total=len(data_infos), desc='计算车道曲率'):
...
def main():
    ...
    for idx in sample_ids:
        ...
        visualize_sample_fixed(data_infos[idx], idx, NUM_SAMPLE_POINTS, OUTPUT_DIR, IMG_W, IMG_H)
