...
class LLANetHeadWithStaticsPriors(nn.Module):
    def __init__(
        ...
        if self.enable_category:
            cat_weights = None
            if self.stats_data is not None and 'cls_category_weights' in self.stats_data:
                # The loaded weights are actually frequencies. We need to invert them to get proper loss weights.
                freqs = torch.tensor(self.stats_data['cls_category_weights'], dtype=torch.float32)
                # Invert: 1 / (freq + epsilon)
                cat_weights = 1.0 / (freqs + 1e-6)
                # Normalize so mean is 1
                cat_weights = cat_weights / cat_weights.mean()

                self.logger.info(f'Using statistics for category loss weights. Weights: {cat_weights}')
            self.category_criterion = torch.nn.NLLLoss(weight=cat_weights, reduction='sum')
    ...
    def __init_prior_statistics(self):
        ...
        if self.dataset_statistics and os.path.exists(self.dataset_statistics):
            try:
                ...
                self.logger.info(f'Loaded dataset statistics: {self.dataset_statistics}')
    ...
    def __init_buffers(self):
        ...
        if hasattr(self.cfg, 'sample_y') and self.cfg.sample_y is not None:
            try:
                ...
                self.logger.info('Init prior Y buffers using external sample_y from config.')
                use_external = True
            except Exception as e:
        ...
        if not use_external and self.stats_data is not None:
            try:
                has_y = 'start_y_grid' in self.stats_data and 'start_y_pdf' in self.stats_data
                has_x = 'start_x_grid' in self.stats_data and 'start_x_pdf' in self.stats_data
                if has_y:
                    y_grid = np.array(self.stats_data['start_y_grid']).astype(np.float32)
                    y_pdf = np.array(self.stats_data['start_y_pdf']).astype(np.float32)
                    y_grid = np.clip(y_grid, 0.0, 1.0)
                    y_pdf = np.maximum(y_pdf, 0.0)
                    y_pdf_sum = float(np.sum(y_pdf)) if float(np.sum(y_pdf)) > 0 else 1.0
                    ...
                    self.register_buffer(name='prior_ys', tensor=torch.from_numpy(1.0 - y_off).float())
                    self.register_buffer(name='start_y_pdf_grid', tensor=torch.from_numpy(y_grid).float())
                    self.register_buffer(name='start_y_pdf', tensor=torch.from_numpy(y_pdf).float())
                    use_pdf = True
                    self.logger.info(
                        f'Init prior Y buffers from PDF: feat_points={self.sample_points}, offsets={self.n_offsets}'
                    )
                if has_x:
                    x_grid = np.array(self.stats_data['start_x_grid']).astype(np.float32)
                    x_pdf = np.array(self.stats_data['start_x_pdf']).astype(np.float32)
                    x_grid = np.clip(x_grid, 0.0, 1.0)
                    x_pdf = np.maximum(x_pdf, 0.0)
                    self.register_buffer(name='start_x_pdf_grid', tensor=torch.from_numpy(x_grid).float())
                    self.register_buffer(name='start_x_pdf', tensor=torch.from_numpy(x_pdf).float())
        ...
        if not use_external and not use_pdf:
            ...
            self.register_buffer(
                name='prior_ys',
                tensor=torch.linspace(1.0, 0.0, steps=self.n_offsets, dtype=torch.float32),
            )
            self.logger.info('Init prior Y buffers using uniform linspace (Top-to-Bottom).')
        self.logger.info(f'Init buffers: sample_points={self.sample_points}, n_offsets={self.n_offsets}')
        self.logger.info(
            ...
            if self.stats_data is not None and 'seg_positive_ratios' in self.stats_data:
                try:
                    ...
                    if len(pos_ratios) > 0:
                        ...
                        self.logger.info(f'Init cls bias using prior seg_positive_ratios, prior_prob={prior_prob:.5f}')
                except Exception as e:
    ...
    def _init_prior_embeddings(self):
        ...
        if self.stats_data is not None and 'cluster_centers' in self.stats_data:
            ...
            raise ValueError(
                "No statistics found or 'cluster_centers' missing. LLANetHeadWithStaticsPriors requires valid statistics."
            )
    ...
    def pool_prior_features(self, batch_features, num_priors, prior_xs):
        ...
        prior_ys = self.prior_feat_ys.view(1, 1, -1, 1).expand(batch_size, num_priors, -1, 1)
    ...
    def forward(self, features, img_metas=None, **kwargs):
        ...
        prior_ys_expanded = self.prior_ys.to(device).view(1, 1, -1).expand(batch_size, self.num_priors, -1)
        ...
        for stage in range(self.refine_layers):
            ...
            if torch.isnan(batch_prior_features).any():
                self.logger.error(f'NaN detected in batch_prior_features at stage {stage}!')
            ...
            reg = self.reg_layers(reg_features).reshape(batch_size, -1, self.n_offsets + 1 + 2 + 1)
            ...
            clamped_len = pred_len  # Length is unnormalized (points), do not clamp to 1.0
    ...
    def get_lanes(self, output, as_lanes=True):
        ...
        for predictions in output:
            ...
            self.logger.debug(f'DEBUG: Keeping {keep_inds.sum()} lanes after threshold {threshold}')
    ...
    def _normalize_targets(self, batch_targets):
        ...
        if batch_targets.numel() > 0:
            ...
            mask_theta = batch_targets[..., 4] > 2.0  # Assume > 2 degrees is unnormalized
            if mask_theta.any():
    ...
    def loss(self, outputs, batch, current_iter=0):
        ...
        for stage in range(self.refine_layers):
            ...
            if num_positives > 0:
                ...
                # ================== 【增强版可视化】 ==================
                if current_iter % 50 == 0 and torch.distributed.get_rank() == 0 and stage == self.refine_layers - 1:
