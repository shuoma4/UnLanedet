...
from unlanedet.model.module.head import PlainDecoder
from unlanedet.model.module.losses import FocalLoss

from .modules.dynamic_assign import dynamic_assign
from .modules.line_iou import liou_loss
from .modules.roi_gather import LinearModule, ROIGather


class LLANetV2(Detector):
...
class LLANetV2Head(nn.Module):
    def __init__(
        ...
        super(LLANetV2Head, self).__init__()
        self.cfg = cfg
    ...
    def predictions_to_pred(self, predictions):
        ...
        for lane in predictions:
            ...
            mask = ~(
                (((lane_xs[:start] >= 0.0) & (lane_xs[:start] <= 1.0)).cpu().numpy()[::-1].cumprod()[::-1]).astype(
                    np.bool
                )
            )
            lane_xs[end + 1 :] = -2
    ...
    def loss(
        ...
        for stage in range(self.refine_layers):
            ...
            # Optimized Loop: Still iterate over batch, but use optimized assign
            # Vectorizing over batch is hard because number of targets differs per image
            # But assignment is per-image anyway.
            # We can parallelize this loop if we padded targets, but here we keep batch loop
            # and rely on vectorized assignment within the loop.

            for predictions, target in zip(predictions_list, targets):
                # Filter valid targets (label == 1)
                ...
                with torch.no_grad():
                    # Use optimized dynamic_assign
                    matched_row_inds, matched_col_inds = dynamic_assign(predictions, target, self.img_w, self.img_h)
                ...
                with torch.no_grad():
                    predictions_starts = torch.clamp(
                        (predictions[matched_row_inds, 2] * self.n_strips).round().long(),
                        0,
                        self.n_strips,
