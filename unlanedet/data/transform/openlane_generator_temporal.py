        ...
        for i in range(T):
            ...
            if "cut_height" in sample and sample["cut_height"] != 0:
                ...
                for lane in sample["lanes"]:
                    new_lanes.append(
                        [(p[0], p[1] - sample["cut_height"]) for p in lane]
                    )
            ...
            if self.training:
                ...
                if sample.get("mask") is None:
                    print(
                        f"WARNING: mask is None for sample {sample.get('img_path', 'unknown')}. Creating dummy mask.",
                        flush=True,
                    )
            ...
            out_sample["seg"] = (
                seg.get_arr()
                if self.training
                else np.zeros(img_org.shape[:2], dtype=np.uint8)
            )
        ...
        for k in keys:
            if k in stackable_keys:
                try:
                    ...
                    batched_dict[k] = np.stack(batched_items, axis=0) if isinstance(batched_items[0], np.ndarray) else batched_items
                except Exception:
    ...
    def __call__(self, sample):
        ...
        for key in sample.keys():
            ...
            if key in self.collect_keys:
                if "seq_" in key or key in ["xyz", "visibility", "track_id"]:
                    data[key] = DataContainer(sample[key], cpu_only=True)
                else:
                    data[key] = sample[key]
        return data
    ...
    def __call__(self, sample):
        ...
        if isinstance(img, np.ndarray):
            if len(m) == 1:
                ...
                img = (img - m[np.newaxis, np.newaxis, np.newaxis, :]) / s[
