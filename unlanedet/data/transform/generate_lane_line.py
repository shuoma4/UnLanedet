...
# Monkey patch to fix potential issue with LUT table size in imgaug and ensure cache init
try:

    def _generate_lut_table_patched(cls):
        dtype = np.uint8
    ...
    color.AddToHueAndSaturation._generate_lut_table = classmethod(_generate_lut_table_patched)
    # Initialize cache immediately to ensure it exists in workers
    color.AddToHueAndSaturation._LUT_CACHE = color.AddToHueAndSaturation._generate_lut_table()

    # Monkey patch to fix np.bool deprecation in imgaug
    def _get_augmenter_active_patched(self, nb_rows, random_state):
    ...
    meta.Augmenter._get_augmenter_active = _get_augmenter_active_patched
    # Also patch OneOf specifically as it might override or use the base implementation
    # But since OneOf inherits from Augmenter (via SomeOf -> Augmenter), patching Augmenter might not be enough if OneOf overrides it.
    # Checking imgaug source code, SomeOf (parent of OneOf) implements _get_augmenter_active.

    def _get_augmenter_active_someof_patched(self, nb_rows, random_state):
        # pylint: disable=invalid-name
        nn = self._get_n(nb_rows, random_state)
        nn = [min(n, len(self)) for n in nn]
        augmenter_active = np.zeros((nb_rows, len(self)), dtype=bool)
    ...
    meta.SomeOf._get_augmenter_active = _get_augmenter_active_someof_patched

except Exception as e:
    print(f'Warning: Failed to patch imgaug: {e}')
...
class GenerateLaneLine(object):
    ...
    def transform_annotation(self, anno, img_wh=None):
        ...
        old_lanes = [
            [[x * self.img_w / float(img_w), y * self.img_h / float(img_h)] for x, y in lane] for lane in old_lanes
        ]
        # create tranformed annotations
        lanes = (
            np.ones((self.max_lanes, 2 + 1 + 1 + 2 + self.n_offsets), dtype=np.float32) * -1e5
        ...
        for lane_idx, lane in enumerate(old_lanes):
            ...
            for i in range(1, len(xs_inside_image)):
                theta = math.atan(i * self.strip_size / (xs_inside_image[i] - xs_inside_image[0] + 1e-5)) / math.pi
        ...
        for i in range(30):
            if self.training:
                ...
                img, line_strings, seg = self.transform(
                    image=img_org.copy().astype(np.uint8), line_strings=line_strings_org, segmentation_maps=mask_org
                )
                # print(44444)
...
class GenerateLaneLineATT(object):
    def __init__(self, transforms=None, wh=(640, 360), cfg=None):
        ...
        if transforms is not None:
            transforms = [
                getattr(iaa, aug['name'])(**convert_dictconfig_to_dict(aug)['parameters']) for aug in transforms
            ]  # add augmentation
        else:
            transforms = []
    ...
    def transform_annotation(self, anno, img_wh=None):
        ...
        # create tranformed annotations
        lanes = (
            np.ones((self.max_lanes, 2 + 1 + 1 + 1 + self.n_offsets), dtype=np.float32) * -1e5
        )  # 2 scores, 1 start_y, 1 start_x, 1 length, S+1 coordinates
        # lanes are invalid by default
        lanes[:, 0] = 1
        lanes[:, 1] = 0
        for lane_idx, lane in enumerate(old_lanes):
    ...
    def __call__(self, sample):
        ...
        sample['img'] = (img / 255.0).astype(np.float32)
        sample['lane_line'] = label
...
class GenerateSRLaneLine(object):
    ...
    def sample_lane(points, sample_ys):
        ...
        assert len(sample_ys_inside_domain) > 0
        interp_xs = interp(
            sample_ys_inside_domain
        )  # Since it is interpolation, the interp_xs are guaranteed to be within the range of the image. # noqa: E501
    ...
    def check_horizontal_lane(points, angle_threshold=5):
        ...
        if len(points) < 2:
            return False
        rad = math.atan(math.fabs((points[-1][1] - points[0][1]) / (points[0][0] - points[-1][0] + 1e-6)))
    ...
    def generate_angle_map(self, lanes):
        ...
        for stride in self.feat_ds_strides:
            ...
            for lane_idx, lane in enumerate(lanes, 1):
                try:
                    all_xs = self.sample_lane(lane, offsets_ys)
                except AssertionError:
                    continue
                all_xs = all_xs / stride
                offsets_ys_down = offsets_ys / stride
                for i, (x, y) in enumerate(zip(all_xs[1:], offsets_ys_down[1:]), 1):
                    int_x, int_y = int(x), int(y)
                    if int_x < 0 or int_x >= gt_angle.shape[1] or int_y < 0 or int_y >= gt_angle.shape[0]:
                        continue
    ...
    def transform_annotation(self, old_lanes):
        ...
        # removing lanes with less than 2 points
        old_lanes = filter(
            lambda x: len(x) > 1, old_lanes
        )  # The y-coordinate increases from the top of the image downwards. # noqa: E501
        # sort lane points by Y (bottom to top of the image)
        old_lanes = [sorted(lane, key=lambda x: -x[1]) for lane in old_lanes]
        # remove points with same Y (keep first occurrence)
        ...
        lanes = (
            np.ones((self.max_lanes, 2 + 2 + self.n_offsets), dtype=np.float32) * -1e5
        )  # 2 scores, 1 start_y, 1 length, n_offsets coordinates
        # lanes are invalid by default
        lanes[:, 0] = 1
        lanes[:, 1] = 0
        for lane_idx, lane in enumerate(old_lanes):
    ...
    def __call__(self, sample):
        ...
        for i in range(10):
            ...
            line_strings.clip_out_of_image_()
            try:
                annos = self.transform_annotation(self.linestrings_to_lanes(line_strings))
                break
            except Exception as e:
                if (i + 1) == 10:
                    raise Exception(e)
